...
int main(int argc, char* argv[]) {
	...
	do {
		...
		delta_k.resize(N, 0);
		for (int i = 0; i < N; i++) {
			delta_k[i] = k * distribution(generator);
		}

		d_lk_temp.resize(N_arc, 0);
		for (int i = 0; i < N_arc; i++) {
			d_lk_temp[i] = k * distribution(generator);
		}
		double average_val = accumulate(d_lk_temp.begin(), d_lk_temp.end(), 0) / d_lk_temp.size();
		d_lk.resize(N_arc, average_val);
...

		{
			...
			try {
				...
				std::string str3 = "C:\\Users\\akhga\\source\\repos\\DitoLgf\\ICpaper\\2q12." + instance + ".txt"; //Please change the link according to repository you put the feasible solution data.
	...
	save << instance << " , " << time / 1 << " , " << Zcheck << " , " << *min_element(feas_obj.begin(), feas_obj.end()) << " , " << usercut << " , " << ave_ite / 1 << " , " << "( " << al_1[0] << "  " << al_2[0] << " )" << " , " << coverage[0] << " , " << M_ite / 1 << " , " << mip_ite << endl;
	save << ",,,,,," << "( " << al_1[1] << "  " << al_2[1] << " )" << " , " << coverage[1] << endl;
	save << ",,,,,," << "( " << al_1[2] << "  " << al_2[2] << " )" << " , " << coverage[2] << endl;
